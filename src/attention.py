...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, q, k, v, need_weights=True, attn_mask=None, average_attn_weights=True, is_causal=False):
        ...
        if need_weights:
            ...
            attn_output_weights = self.dropout1(attn_output_weights) # (N, H, L, S)
            attn_output = attn_output_weights @ v # (N, H, L, S) @ (N, H, S, E//H) = (N, H, L, E//H)
            if average_attn_weights:
                attn_output_weights.sum(dim=1) / self.num_heads # (N, L, S)
        else: # faster
        ...
        # Note: karpathy has a dropout layer after this, while Torch doesn't
